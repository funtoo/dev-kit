...
src_compile() {
	...
	local conf_mingw64_runtime=( --with-crt --disable-lib32 )
	...
	mwt-build() {
		...
		declare -f mwt-${id} >/dev/null && mwt-${id}
		declare -f mwt-${id}_${2} >/dev/null && mwt-${id}_${2}

		popd >/dev/null || die
	}

	# workaround race condition with out-of-source crt build (bug #879537)
	mkdir -p mingw64_runtime-build/mingw-w64-crt/lib64 || die
	...
	dostrip -x ${mwtdir}/{${CTARGET}/lib,lib/gcc/${CTARGET}}
