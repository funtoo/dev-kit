...
SRC_URI="https://download.jetbrains.com/idea/ideaIU-2020.2.1.tar.gz -> idea-ultimate-2020.2.1.tar.gz"
...
src_install() {
	...
	local svgfile="$(find ${dst}/bin -iname '*.svg')"
	newicon $svgfile "${PN}.svg" || die

	local pngfile="$(find ${dst}/bin -iname '*.png')"
	newicon $pngfile "${PN}.png" || die

	make_desktop_entry ${PN} "IntellJ IDEA Ultimate" ${PN} "Development;IDE;" || die
