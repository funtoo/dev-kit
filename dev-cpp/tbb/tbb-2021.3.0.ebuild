...
SRC_URI="https://api.github.com/repos/oneapi-src/oneTBB/tarball/v2021.3.0 -> tbb-2021.3.0.tar.gz"
...
src_prepare() {
	for file in $(grep -rl mallinfo ${S}) ; do
		grep -q mallinfo2 "${file}" || sed -i 's/mallinfo/mallinfo2/g' "${file}"
	done
	cmake_src_prepare
}
