...
fix_src_dirs() {
	...
	mv google-googletest-* googletest-1.11.0
	popd
}
...
src_prepare() {
	...
	if use test; then
		sed -i 's/-Werror//g' \
			"${WORKDIR}"/googletest-1.11.0/googletest/cmake/internal_utils.cmake || die
	fi
}
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DABSL_LOCAL_GOOGLETEST_DIR="${WORKDIR}/googletest-1.11.0"
