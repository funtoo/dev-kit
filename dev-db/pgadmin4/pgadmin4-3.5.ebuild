...
src_install() {
	dobin pgAdmin4
	newbin - pgAdmin4-web << EOF
#!/bin/sh
exec "${PYTHON}" "${EPREFIX}/usr/share/${PN}/web/pgAdmin4.py"
EOF
	...
	cat > "${D}${APP_DIR}"/config_distro.py <<-EOF
		SERVER_MODE = False
...
pkg_postinst() {
	einfo "pgAdmin4-web binary can be used for starting web-server. To access,"
	einfo "use 127.0.0.1:5050 in browser. This is, however, not recommemded upstream."
	einfo "Please, follow, https://www.pgadmin.org/docs/pgadmin4/3.x/server_deployment.html"
	einfo "When using pgAdmin4-web, default PATH's are set to ~/.pgadmin"
}
